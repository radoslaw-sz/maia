...
class Session:
    ...
    async def agent_says(self, from_agent: str, to_agent: str, message: str) -> "Session":
        ...
        msg = Message(content=message, sender=from_agent, metadata={"to_agent": to_agent})
        self.bus.add_message(msg)
        return self
    ...
    async def run_agent_conversation(self, initiator: str, responder: str, initial_message: str, max_turns: int) -> List[Message]:
        ...
        # Initial message
        msg = Message(content=initial_message, sender=initiator, metadata={"to_agent": responder})
        self.bus.add_message(msg)
        ...
        for _ in range(max_turns):
            ...
            if response_to_initiator:
                msg = Message(content=response_to_initiator.content, sender=responder, metadata={"to_agent": initiator})
                self.bus.add_message(msg)
                conversation_log.append(msg)
            else:
                break
            ...
            if response_to_responder:
                msg = Message(content=response_to_responder.content, sender=initiator, metadata={"to_agent": responder})
                self.bus.add_message(msg)
                conversation_log.append(msg)
            else:
                break
